...
//only month and day can be searched - EHR-874
let searchingKeyword = '03'

describe('list-view-vitals', () => {
  ...
  //opening vitals list view
  it('should open vitals list tab', async () => {
    ...
    await page.fill('[data-testid="search-patients"]', 'vitals')

    await page.waitForTimeout(3000)

    await page.hover(
      '(//*[@data-testid="patients-table"])/div/div/table/tbody/tr[2]/td[1]'
    )
  ...
  //searching vitals that does not exist
  it('should display no data', async () => {
    pageNumberCounter = await page.innerText(
      '(//*[@data-testid="vitals-table"])/following-sibling::ul[1]/li[@title="Next Page"]/preceding-sibling::li[1]'
    )
    await page.waitForSelector('[data-testid="search-vitals"]')
    await page.fill('[data-testid="search-vitals"]', 'ksklsajflksa')

    await page.waitForTimeout(3000)
    let noDataStateValue = await page.innerText(
      '(//*[@data-testid="vitals-table"])/div/div/table/tbody/tr[2]'
    )

    await page.waitForSelector('[data-testid="search-vitals"]')
    await page.fill('[data-testid="search-vitals"]', searchingKeyword)
  ...
  //searching any vitals
  it('should search any vital data entered in search box', async () => {
    ...
    allRecords = await page.innerText(
      '(//*[@data-testid="vitals-table"])/div/div/table/tbody'
    )
    ...
    if (pageNumberCounter != 1) {
      for (
        ...
        await page.click(
          '(//*[@data-testid="vitals-table"])/following-sibling::ul[1]/li[@title="Next Page"]'
        )
  ...
  it('should validate number of records per page', async () => {
    //clearing the search
    await page.waitForSelector('[data-testid="search-vitals"]')
    await page.fill('[data-testid="search-vitals"]', '')
    ...
    if (pageNumberCounter > 5) {
      ...
      for (
        ...
        await page.click(
          '(//*[@data-testid="vitals-table"])/following-sibling::ul[1]/li[@title="Next Page"]/following-sibling::li'
        )
  ...
  it('should display detail view', async () => {
    ...
    let vitalsRecordingsFor = await page.innerText(
      '(//*[@data-testid="vitals-table"])/div/div/table/tbody/tr[' +
        randomValue +
        ']/td[1]'
    )
    ...
    await page.waitForSelector('[data-testid="vitals-details-title"]')

    let vitalsTitleOfDetailView = await page.innerText(
      '[data-testid="vitals-details-title"]'
    )

    expect(vitalsTitleOfDetailView.includes(vitalsRecordingsFor)).toBe(true)
  })
})
